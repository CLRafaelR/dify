...
class IterationNode(BaseIterationNode):
    ...
    async def _run(self, variable_pool: VariablePool) -> BaseIterationState:
        """
        Run the node.
        """
        ...
        if not isinstance(iterator, list):
            raise ValueError(
                f"Invalid iterator value: {iterator}, please provide a list."
            )
        ...
        state = IterationState(
            ...
            outputs=[],
            metadata=IterationState.MetaData(
                iterator_length=len(iterator) if iterator is not None else 0
            ),
        )

        await self._set_current_iteration_variable(variable_pool, state)
        return state

    async def _get_next_iteration(
        self, variable_pool: VariablePool, state: IterationState
    ) -> NodeRunResult | str:
        """
        ...
        await self._resolve_current_output(variable_pool, state)
        # move to next iteration
        await self._next_iteration(variable_pool, state)

        node_data = cast(IterationNodeData, self.node_data)
        if self._reached_iteration_limit(variable_pool, state):
            return NodeRunResult(
                status=WorkflowNodeExecutionStatus.SUCCEEDED,
                outputs={"output": jsonable_encoder(state.outputs)},
            )

        return node_data.start_node_id

    async def _set_current_iteration_variable(
        self, variable_pool: VariablePool, state: IterationState
    ):
        """
    ...
    async def _next_iteration(self, variable_pool: VariablePool, state: IterationState):
        """
        Move to next iteration.
    ...
    async def _resolve_current_output(
        self, variable_pool: VariablePool, state: IterationState
    ):
        """
        Resolve current output.
